...
function addService(manifest) {
  ...
  const exists = app.service.find(
    (s) => s.$['android:name'] === 'com.shaho.popli.reelupload.ReelUploadService',
  );
  if (!exists) {
    app.service.push({
      $: {
        'android:name': 'com.shaho.popli.reelupload.ReelUploadService',
...
const withReelUploadMainApplication = (config) => {
  return withMainApplication(config, (config) => {
    ...
    contents = contents.replace(
      ...
      'import expo.modules.ExpoReactHostFactory\nimport com.shaho.popli.reelupload.ReelUploadPackage',
    );
  ...
  return withDangerousMod(config, [
    ...
    (config) => {
      ...
      const dest = path.resolve(
        ...
        'app/src/main/java/com/shaho/popli/reelupload',
      );
