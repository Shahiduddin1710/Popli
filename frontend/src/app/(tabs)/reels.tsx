...
import { ChevronLeft, PartyPopper, Sparkles, Star, Heart } from 'lucide-react-native';
      ...
      {burstGift.visible && (
        <View style={StyleSheet.absoluteFill} className="items-center justify-center bg-black/70 z-50" pointerEvents="none">
          ...
          />
               ...
               {[
            { x: -100, y: -150, d: 0, icon: <Sparkles size={24} color="#FBBF24" /> },
            { x: 120, y: -200, d: 100, icon: <PartyPopper size={24} color="#F472B6" /> },
            { x: -150, y: 50, d: 200, icon: <Star size={24} color="#A78BFA" /> },
            { x: 140, y: 80, d: 300, icon: <Heart size={24} color="#F87171" /> },
            { x: 0, y: -250, d: 400, icon: <PartyPopper size={24} color="#34D399" /> },
          ].map((confetti, i) => (
            <MotiView
              key={i}
