...
import { ArrowLeft, Coins, ArrowUpRight, TrendingUp, Landmark, Wallet, ArrowDownLeft, Clock, CheckCircle2, ListFilter, AlertCircle, CircleDollarSign } from 'lucide-react-native';
          ...
          <View className="gap-3 mt-2">
            ...
            <Text className={`font-black text-xs flex-row items-center ${isIncome ? 'text-accent-green' : 'text-red-400'}`}>
                        ...
                        <View className="flex-row items-center gap-1">
                          <Text className={`font-black text-xs ${isIncome ? 'text-accent-green' : 'text-red-400'}`}>{`${isIncome ? '+' : '-'}${amount.toFixed(0)}`}</Text>
                          <CircleDollarSign size={10} color={isIncome ? '#4ADE80' : '#F87171'} />
                        </View>
                      ) : `${isIncome ? '+' : '-'}${formatEarnings(amount)}`}
                    </Text>
