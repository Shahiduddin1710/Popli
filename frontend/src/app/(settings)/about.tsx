...
const LinkRow = ({ icon: Icon, title, onPress }: any) => (
  <Pressable onPress={onPress} className="flex-row items-center justify-between border-b border-white/5 py-4">
...
export default function AboutScreen() {
 const router = useRouter();
  ...
  const openLegal = (type: string) => router.push({ pathname: '/(auth)/legal', params: { type } });
    ...
    <View className="flex-1 bg-[#12081E] pt-14">
      ...
      >
        ...
        <View className="gap-4">
          ...
          <LinkRow icon={FileText} title="Terms of Service" onPress={() => openLegal('terms')} />
            <LinkRow icon={ShieldCheck} title="Privacy Policy" onPress={() => openLegal('privacy')} />
            <LinkRow icon={Users} title="Community Guidelines" onPress={() => openLegal('guidelines')} />
          </View>
        </View>
